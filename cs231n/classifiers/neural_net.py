...
class TwoLayerNet(object):
    ...
    def loss(self, X: np.ndarray, y=None, reg=0.0):
        ...
        loss -= np.sum(s[range(N),y])
        loss += np.sum(np.log(np.exp(s).sum(1)))
        loss /= N
        ...
        out /= N
        grads['b2'] = out.sum(0)
        grads['W2'] = relu.T @ out + reg * W2
        dh = out @ W2.T
        dh[relu == 0] = 0
        grads['W1'] = X.T @ dh + reg * W1
        grads['b1'] = dh.sum(0)"""
        
        output = np.exp(s)/np.sum(np.exp(s),axis=1,keepdims=True)
        output[range(N),y] -= 1
        output /= N
        
        grads['b2'] = output.sum(0)
        grads['W2'] = relu.T @ output + reg * W2
